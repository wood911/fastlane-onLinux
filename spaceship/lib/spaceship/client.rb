...
module Spaceship
  ...
  class Client
    ...
    def parse_response(response, expected_key = nil)
      ...
      if content.nil?
        detect_most_common_errors_and_raise_exceptions(response.body, response.status) if response.body
        Utils.error_callback(604, "UnexpectedResponse Error, http.status=#{response.status}", response.body)
    ...
    def detect_most_common_errors_and_raise_exceptions(body, status = 601)
      ...
      if body["messages"] && body["messages"]["error"].include?("Forbidden")
        ...
        error_msg = "Received an internal server error from App Store Connect / Developer Portal, please try again later"
        Utils.error_callback(status, error_msg, body)
        raise InternalServerError, error_msg
      elsif body.to_s.include?("Gateway Timeout - In read")
        error_msg = "Received a gateway timeout error from App Store Connect / Developer Portal, please try again later"
        Utils.error_callback(status, error_msg, body)
        raise GatewayTimeoutError, error_msg
      elsif (body["userString"] || "").include?("Program License Agreement")
        error_msg = "#{body['userString']} Please manually log into your Apple Developer account to review and accept the updated agreement."
        Utils.error_callback(602, error_msg, body)
        raise ProgramLicenseAgreementUpdated, error_msg
      elsif (body["userString"] || "").include?("Your developer account needs to be updated") # account close 封号了
        error_msg = "#{body['userString']} account close"
        Utils.error_callback(603, error_msg, body)
        raise ProgramLicenseAgreementUpdated, error_msg
      end
    end
