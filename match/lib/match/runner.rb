...
module Match
  class Runner
    ...
    def fetch_certificate(params: nil, working_directory: nil, specific_cert_type: nil)
      ...
      certificate_id = File.basename(cert_path).gsub(".cer", "") # Certificate ID
      ...
      if params[:output_path]
        ...
        ENV["CER_FILE_PATH"] = File.join(params[:output_path], File.basename(cert_path))
        ENV["PRIVATE_KEY_PATH"] = File.join(params[:output_path], File.basename(private_key_path))
      end
    ...
    def fetch_provisioning_profile(params: nil, certificate_id: nil, app_identifier: nil, working_directory: nil)
      ...
      if params[:output_path]
        ...
        installed_profile = File.join(params[:output_path], File.basename(profile))
      end

      Utils.profile_callback(parsed)
